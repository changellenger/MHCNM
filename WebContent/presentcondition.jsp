			...
			<p><input type="image" src="images/screen.jpg"  name="screen"  width="450" height="30"></p><br /><br />
						...
						<input type="hidden" value="<%=timet %>" name="timet"> 		
 						...
 						try {
 						    ...
 						    String sql = "Select * from SEAT WHERE THE_NUMBER='"+movie+"' AND TAB_NUMBER='"+timet+"' AND SEAT_DATE='"+date+"' ";
 						   ...
 						   while(rs.next()){
 							...
 							for(int j=0;j<10;j++){%><p><% 
	 					 	for(int i=0;i<10;i++){%>
   					<%if(possible[k]==1){
   					%><span><img src="images/<%=i+1%>.png" width="40" height="40"/></span>
 					<%k++;
 					
 					
   					}else{%>
   						<span><img src="images/gray.png" width="40" height="40"/></span>
 					<%	k++;
 					count++;
 					  }
					}
				...
				<%}
 					%><input type="submit" class=testbutton value="<%=count%>/100">
 					<% }catch(Exception e){
 						 out.println(e);
 						 out.println("실패");
 						}
